...
describe("Contract: StakedCLRToken", async () => {
  ...
  beforeEach(async () => {
    ...
    await lmTerminal.deployUniswapPool(
      token0.address,
      token1.address,
      3000,
      poolPrice
    ...
    await lmTerminal.deployIncentivizedPool(
      ...
      { rewardTokens: [rewardToken.address], vestingPeriod: 0 },
      {
        ...
        amount0: bnDecimal(100000),
        amount1: bnDecimal(100000),
      },
    ...
    stakedToken = await ethers.getContractAt(
      "StakedCLRToken",
      stakedTokenAddress
    );
    ...
    let amts = await clr.calculateAmountsMintedSingleToken(
      0,
      bnDecimals(100000, token0Decimals)
    );
  ...
  describe("Staked token", async () => {
    it(`account should receive staked token on liquidity provision`, async () => {
      ...
      it(`account should burn staked token on liquidity removal`, async () => {
        let stakedTokenBalanceBefore = await stakedToken.balanceOf(
          admin.address
        );
        await clr.withdraw(bnDecimals(1, token0Decimals), 0, 0);
        let stakedTokenBalanceAfter = await stakedToken.balanceOf(
          admin.address
        );
      ...
      it(`account shouldn't be able to transfer staked token`, async () => {
        await expect(
          stakedToken.transfer(user1.address, bnDecimal(10000000))
      ...
      it(`account shouldn't be able to call mint on staked token`, async () => {
        await expect(
          stakedToken.mint(admin.address, bnDecimal(10000000))
      ...
      it(`should be able to transfer staked token if it's initialized as transferable`, async () => {
        let stakedToken2 = await deploy("StakedCLRToken");
        let stakedToken2Proxy = await deployArgs(
          "StakedCLRTokenProxy",
          stakedToken2.address,
          user3.address, // placeholder for proxyAdmin
          user3.address // placeholder for clrDeployer
        );
        stakedToken2 = await ethers.getContractAt(
          "StakedCLRToken",
          stakedToken2Proxy.address
        );
        await stakedToken2.initialize(
          "StakedToken",
          "SCLRToken",
          admin.address,
          true
