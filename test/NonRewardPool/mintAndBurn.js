...
const { bn, bnDecimal, bnDecimals, increaseTime, swapToken0ForToken1Decimals, swapToken1ForToken0Decimals } = require('../../scripts/helpers');
const { expect } = require('chai');

// Mint and burn tests for NonRewardPool
describe('Contract: NonRewardPool', async () => {
  ...
  beforeEach(async () => {
      ({ lmTerminal, token0, token1, rewardToken, nonRewardPool, router,
  ...
  describe('Mint and burn', async () => {
    it('should mint nonRewardPool tokens to user with asset 0', async () => {
        let amount = bnDecimals(1000000, token0Decimals);
        let mint = await nonRewardPool.calculateAmountsMintedSingleToken(0, amount);
        await nonRewardPool.connect(user1).deposit(mint.amount0Minted, mint.amount1Minted);
        let balance = await nonRewardPool.balanceOf(user1.address);

        let am = await nonRewardPool.calculateAmountsMintedSingleToken(0, amount);
        let liquidityAmount = await nonRewardPool.getLiquidityForAmounts(am.amount0Minted, am.amount1Minted);
        let positionLiquidity = await nonRewardPool.getPositionLiquidity();

        const totalSupply = await nonRewardPool.totalSupply();
        let calculatedBalance = bn(liquidityAmount).mul(totalSupply).div(positionLiquidity);

        expect(balance).to.eq(calculatedBalance);
    }),

    it('should mint nonRewardPool tokens to user with asset 1', async () => {
        let amount = bnDecimals(1000000, token1Decimals);
        let mint = await nonRewardPool.calculateAmountsMintedSingleToken(1, amount);
        await nonRewardPool.connect(user1).deposit(mint.amount0Minted, mint.amount1Minted);
        let balance = await nonRewardPool.balanceOf(user1.address);
    ...
    it('should stake tokens in position when minting', async () => {
        let amount = bnDecimals(1000000, token0Decimals);
        let am = await nonRewardPool.calculateAmountsMintedSingleToken(0, amount);
        let liquidity = await nonRewardPool.getLiquidityForAmounts(
                              am.amount0Minted, am.amount1Minted)

        let positionLiquidityBefore = await nonRewardPool.getPositionLiquidity();
        await nonRewardPool.connect(user1).deposit(am.amount0Minted, am.amount1Minted);
        let positionLiquidityAfter = await nonRewardPool.getPositionLiquidity();
    ...
    it('should deposit in pool ratio if token 0\'s balance hasn\'t changed by more than 1%', async() => {
      ...
      let expectedDepositAmounts = await nonRewardPool.calculatePoolMintedAmounts(mint.amount0Minted, mint.amount1Minted);
      ...
      await expect(nonRewardPool.connect(user1).deposit(mint.amount0Minted, mint.amount1Minted)).
    ...
    it('shouldn\'t allow user to mint if he has no token balance', async () => {
      ...
      let mint = await nonRewardPool.calculateAmountsMintedSingleToken(0, mintAmount);
      expect(await token0.balanceOf(user4.address)).to.equal(0);
      await expect(nonRewardPool.connect(user4).deposit(mint.amount0Minted, mint.amount1Minted)).to.be.reverted;
    }),

    it('shouldn\'t allow user to mint with 0 amount', async () => {
      await expect(nonRewardPool.deposit(0, 0)).to.be.reverted;
    }),

    it('shouldn\'t allow minting if contract is paused', async () => {
      await nonRewardPool.pauseContract();
      let mintAmount = bnDecimals(10000, token0Decimals);
      await expect(nonRewardPool.connect(user1).deposit(0, mintAmount)).
            to.be.revertedWith('Pausable: paused');
    }),

    it('should burn nonRewardPool tokens from user when burning', async () => {
      ...
      let balanceBefore = await nonRewardPool.balanceOf(user1.address);
      await nonRewardPool.connect(user1).withdraw(burnAmount, 0, 0)
      let balanceAfter = await nonRewardPool.balanceOf(user1.address);
    ...
    it('should transfer asset balance back to user when burning', async () => {
      ...
      const positionBalance = await nonRewardPool.getStakedTokenBalance();
      const totalSupply = await nonRewardPool.totalSupply();
    ...
    it('should unstake position liquidity on burn', async () => {
        ...
        // liquidity share = burn amount * total liquidity / total supply
        const positionLiquidity = await nonRewardPool.getPositionLiquidity();
        const totalSupply = await nonRewardPool.totalSupply();
        let proRataBalance = burnAmount.mul(positionLiquidity).div(totalSupply);

        let amountsExpected = await nonRewardPool.getAmountsForLiquidity(proRataBalance);
        let amount0Expected = amountsExpected.amount0;
        let amount1Expected = amountsExpected.amount1;
        let liquidityUnstaked = await nonRewardPool.getLiquidityForAmounts(amount0Expected, amount1Expected);

        const positionLiquidityBefore = await nonRewardPool.getPositionLiquidity();
        await nonRewardPool.connect(user1).withdraw(burnAmount, 0, 0)
        const positionLiquidityAfter = await nonRewardPool.getPositionLiquidity();
    ...
    it('shouldn\'t allow user to burn above more than the expected withdrawable amounts', async () => {
      ...
      let burnAmount = bnDecimal(1);
      let amts = await nonRewardPool.calculateWithdrawAmounts(burnAmount);

      // withdraw with 101% of the expected withdraw amounts should be reverted
      await expect(nonRewardPool.withdraw(burnAmount, amts.amount0.mul(101).div(100), amts.amount1.mul(101).div(100))).
      to.be.reverted;

      // withdraw with 99% of the expected amounts shouldn't be reverted
      await expect(nonRewardPool.withdraw(burnAmount, amts.amount0.mul(99).div(100), amts.amount1.mul(99).div(100))).
      not.to.be.reverted;
    }),

    it('shouldn\'t allow user to burn if he hasn\'t minted', async () => {
      let burnAmount = bnDecimals(10000, token0Decimals);
      expect(await nonRewardPool.balanceOf(user2.address)).to.equal(0);
      await expect(nonRewardPool.connect(user1).withdraw(burnAmount, 0, 0)).to.be.reverted;
    }),

    it('shouldn\'t allow user to burn with 0 amount', async () => {
      await expect(nonRewardPool.withdraw(0, 0, 0)).to.be.reverted;
    }),

    it('should revert if burn amount exceeds available liquidity', async () => {
      let poolTotalLiquidity = await nonRewardPool.getPositionLiquidity();
      let burnAmount = poolTotalLiquidity.add(1);
      await expect(nonRewardPool.withdraw(burnAmount, 0, 0)).to.be.reverted;
    })
  })
})
