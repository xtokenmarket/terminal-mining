...
describe('Contract: RewardEscrow', async () => {
    let lmTerminal, token0, token1, rewardToken, rewardToken2, rewardToken3, stakingToken;
    ...
    let singleAssetPool1, singleAssetPool2;
    ...
    beforeEach(async () => {
        ...
        // Deploy single asset pools
          
        // Deploy Pool with 1 reward token and vesting period of 6 weeks
        ...
        singleAssetPool1 = await ethers.getContractAt('SingleAssetPool', singleAssetPoolAddress);
        
        // Deploy Pool with 3 reward tokens and vesting period of 8 weeks
        tx = await lmTerminal.deploySingleAssetPool(
            stakingToken.address,
            { rewardTokens: [rewardToken.address, rewardToken2.address, rewardToken3.address], vestingPeriod: vestingPeriod2 }, 
            { value: lmTerminal.deploymentFee() }
        );
        ...
        singleAssetPool2 = await ethers.getContractAt('SingleAssetPool', singleAssetPoolAddress);
        ...
        await stakingToken.approve(singleAssetPool1.address, bnDecimal(100000000000));
        await stakingToken.approve(singleAssetPool2.address, bnDecimal(100000000000));
        await stakingToken.connect(user1).approve(singleAssetPool1.address, bnDecimal(100000000000));
        await stakingToken.connect(user1).approve(singleAssetPool2.address, bnDecimal(100000000000));
        ...
        await increaseTime(300);
        await singleAssetPool1.stake(bnDecimal(100000));
        await increaseTime(300);
        await singleAssetPool2.stake(bnDecimal(100000));
        await increaseTime(300);

        // Initiate clr reward programs
        ...
        // Initiate single asset pool reward programs
        await lmTerminal.initiateRewardsProgram(singleAssetPool1.address, pool1Rewards, rewardDuration1);
        await lmTerminal.initiateRewardsProgram(singleAssetPool2.address, pool2Rewards, rewardDuration2);
    })

  describe('Reward Escrow - CLR pools', async () => {
      ...
      it('should have set the vesting periods for all incentivized pools', async () => {
            let vestingPeriodPool1 = await rewardEscrow.rewardPoolVestingPeriod(pool1.address);
            let vestingPeriodPool2 = await rewardEscrow.rewardPoolVestingPeriod(pool2.address);
  ...
  describe('Reward Escrow - SingleAsset pools', async () => {
      it('should have added the incentivized pools as reward contracts', async () => {
            let pool1IsReward = await rewardEscrow.isRewardContract(singleAssetPool1.address);
            let pool2IsReward = await rewardEscrow.isRewardContract(singleAssetPool2.address);
            expect(pool1IsReward).to.be.true;
            expect(pool2IsReward).to.be.true;
      }),

      it('should have set the vesting periods for all pools', async () => {
            let vestingPeriodPool1 = await rewardEscrow.rewardPoolVestingPeriod(singleAssetPool1.address);
            let vestingPeriodPool2 = await rewardEscrow.rewardPoolVestingPeriod(singleAssetPool2.address);
            expect(vestingPeriodPool1).to.be.eq(vestingPeriod1);
            expect(vestingPeriodPool2).to.be.eq(vestingPeriod2);
      }),
      ...
      it('should receive reward token on claim', async () => {
            await increaseTime(rewardDuration1 / 10);
            let bb = await rewardToken.balanceOf(rewardEscrow.address);
            await singleAssetPool1.claimReward()
            let ba = await rewardToken.balanceOf(rewardEscrow.address);
            expect(ba).to.be.gt(bb);
      }),
      ...
      it('should append vesting entry for user on claim', async () => {
            ...
            let vestingEntriesCount =
                await rewardEscrow.numVestingEntries(singleAssetPool1.address, rewardToken.address, admin.address);
            expect(vestingEntriesCount).to.be.eq(1);

            let rewardToClaim = await rewardToken.balanceOf(rewardEscrow.address);

            let vestingTime = await rewardEscrow.getNextVestingTime(
                singleAssetPool1.address, rewardToken.address, admin.address);
            let vestingQuantity = await rewardEscrow.getNextVestingQuantity(
                singleAssetPool1.address, rewardToken.address, admin.address);
      ...
      it('should be able to retrieve vesting time and quantity in two ways', async () => {
            ...
            let i = await rewardEscrow.getNextVestingIndex(
                singleAssetPool1.address, rewardToken.address, admin.address);
            let vestingTime = await rewardEscrow.getVestingTime(
                singleAssetPool1.address, rewardToken.address, admin.address, i);
            let vestingQuantity = await rewardEscrow.getVestingQuantity(
                singleAssetPool1.address, rewardToken.address, admin.address, i);
            let vestingEntry = await rewardEscrow.getVestingScheduleEntry(
                singleAssetPool1.address, rewardToken.address, admin.address, i);
            ...
            // Get vesting index and entry for an address which hasn't claimed
            i = await rewardEscrow.getNextVestingIndex(
                singleAssetPool1.address, rewardToken.address, user1.address);
            nextVestingEntry = await rewardEscrow.getNextVestingEntry(
                singleAssetPool1.address, rewardToken.address, user1.address);
      ...
      it('should be able to retrieve all vesting entries for an address', async () => {
            ...
            // Get all vesting entries
            let entries = await rewardEscrow.checkAccountSchedule(singleAssetPool1.address, rewardToken.address, admin.address);

            // Get next vesting entry
            let vestingTime = await rewardEscrow.getNextVestingTime(singleAssetPool1.address, rewardToken.address, admin.address);
            let vestingQuantity = await rewardEscrow.getNextVestingQuantity(singleAssetPool1.address, rewardToken.address, admin.address);
      ...
      it(`should append 2 vesting entries for user on claiming from two different pools 
            ...
            await increaseTime(rewardDuration1 / 10);
            await singleAssetPool1.claimReward()
            await singleAssetPool2.claimReward()

            let vestingEntriesCount =
                await rewardEscrow.numVestingEntries(singleAssetPool1.address, rewardToken.address, admin.address);
            expect(vestingEntriesCount).to.be.eq(1);
            vestingEntriesCount =
                await rewardEscrow.numVestingEntries(singleAssetPool2.address, rewardToken.address, admin.address);
            expect(vestingEntriesCount).to.be.eq(1);
      }),
      ...
      it(`should make 3 different vesting entries for each token for a pool
            ...
            expect(vestingEntriesCount).to.be.eq(1);
            vestingEntriesCount =
                await rewardEscrow.numVestingEntries(singleAssetPool2.address, rewardToken2.address, admin.address);
            expect(vestingEntriesCount).to.be.eq(1);
            vestingEntriesCount =
                await rewardEscrow.numVestingEntries(singleAssetPool2.address, rewardToken3.address, admin.address);
            expect(vestingEntriesCount).to.be.eq(1);
      }),
      ...
      it(`shouldn't be able to append a vesting entry if 
                calling from any address other than clr pool`, async () => {
            await expect(rewardEscrow.appendVestingEntry(rewardToken.address, admin.address, 
                                                        singleAssetPool1.address, bnDecimal(10000))).
                to.be.revertedWith('Only reward contract can perform this action');
      }),
      ...
      it(`address should be able to vest after the given vesting period passes`, async () => {
            ...
            await increaseTime(Number(vestingPeriod1));
            let bb = await rewardToken.balanceOf(admin.address);
            await rewardEscrow.vest(singleAssetPool1.address, rewardToken.address);
            let ba = await rewardToken.balanceOf(admin.address);
            let gained = ba.sub(bb);
            expect(gained).to.be.eq(rewardQuantity);
      }),
      ...
      it(`address should be able to vest for multiple tokens after the given vesting period passes`, async () => {
            ...
            let rewardQuantity1 = await rewardEscrow.getNextVestingQuantity(
                singleAssetPool2.address, rewardToken.address, admin.address);
            let rewardQuantity2 = await rewardEscrow.getNextVestingQuantity(
                singleAssetPool2.address, rewardToken2.address, admin.address);
            let rewardQuantity3 = await rewardEscrow.getNextVestingQuantity(
                singleAssetPool2.address, rewardToken3.address, admin.address);
            await increaseTime(Number(vestingPeriod2));

            let bb = await rewardToken.balanceOf(admin.address);
            await rewardEscrow.vest(singleAssetPool2.address, rewardToken.address);
            let ba = await rewardToken.balanceOf(admin.address);
            let gained1 = ba.sub(bb);

            let bb2 = await rewardToken2.balanceOf(admin.address);
            await rewardEscrow.vest(singleAssetPool2.address, rewardToken2.address);
            let ba2 = await rewardToken2.balanceOf(admin.address);
            let gained2 = ba2.sub(bb2);

            let bb3 = await rewardToken3.balanceOf(admin.address);
            await rewardEscrow.vest(singleAssetPool2.address, rewardToken3.address);
            let ba3 = await rewardToken3.balanceOf(admin.address);
            let gained3 = ba3.sub(bb3);
      ...
      it(`address should be able to vest for multiple tokens using vestAll after the given vesting period passes`, async () => {
        ...
        let bb2 = await rewardToken2.balanceOf(admin.address);
        let bb3 = await rewardToken3.balanceOf(admin.address);
        await rewardEscrow.vestAll(singleAssetPool2.address, [rewardToken.address, rewardToken2.address, rewardToken3.address]);
        let ba = await rewardToken.balanceOf(admin.address);
        let gained1 = ba.sub(bb);
        let ba2 = await rewardToken2.balanceOf(admin.address);
        let gained2 = ba2.sub(bb2);
      ...
      it(`address should be able to get the next vesting entry after vesting`, async () => {
            ...
            // get vesting index
            let i = await rewardEscrow.getNextVestingIndex(singleAssetPool1.address, rewardToken.address, admin.address);
            expect(i).not.to.eq(0);
      ...
      it(`Adding and removing contracts multiple times in a row should emit only a single event`, async () => {
          ...
          // Add rewards contract
          await newRewardEscrow.addRewardsContract(singleAssetPool1.address);
          let pool1IsReward = await newRewardEscrow.isRewardContract(singleAssetPool1.address);
          expect(pool1IsReward).to.be.eq(true);

          expect(await newRewardEscrow.addRewardsContract(singleAssetPool1.address)).
            not.to.emit(newRewardEscrow, 'RewardContractAdded')

          // Remove rewards contract
          await newRewardEscrow.removeRewardsContract(singleAssetPool1.address);
          pool1IsReward = await newRewardEscrow.isRewardContract(singleAssetPool1.address);
          expect(pool1IsReward).to.be.eq(false);
          expect(await newRewardEscrow.removeRewardsContract(singleAssetPool1.address)).
            not.to.emit(newRewardEscrow, 'RewardContractRemoved')
      }),
      ...
      it(`shouldn't be able to append entries without enough token balance`, async () => {
          ...
          await expect(newRewardEscrow.appendVestingEntry(
            rewardToken.address, admin.address, singleAssetPool1.address, bnDecimal(100000))).
            to.be.revertedWith('Not enough balance in the contract to provide for the vesting entry');
      }),
      ...
      it(`address shouldn't be able to append more than MAX_VESTING_ENTRIES limit`, async () => {
            ...
            expect(vestingEntriesCount).to.be.eq(MAX_VESTING_ENTRIES + 1);
            await increaseTime(rewardDuration1 / timeInterval);
            await expect(singleAssetPool1.claimReward()).
                to.be.revertedWith('Vesting schedule is too long');
      })
  })
})
